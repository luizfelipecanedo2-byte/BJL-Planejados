...
const ServiceExpensesTab = ({
    ...
    const automaticExpenses = useMemo(() => {
        const projects: Record<string, { income: number, expense: number, client: string }> = {};
        transactions.forEach(t => {
            if (t.orderService) {
                if (!projects[t.orderService]) {
                    projects[t.orderService] = { income: 0, expense: 0, client: t.contact || "Não informado" };
                }
                if (t.type === 'income') projects[t.orderService].income += t.amount;
                else projects[t.orderService].expense += t.amount;
            }
        });
    ...
    const totalAutoIncome = automaticExpenses.reduce((acc, p) => acc + p.income, 0);
    const totalAutoExpense = automaticExpenses.reduce((acc, p) => acc + p.expense, 0);

    return (
        <div className="space-y-10">
            {/* Seção de Gastos Automáticos (Vinculados a OS) */}
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-primary/5 p-6 rounded-2xl border border-primary/20 shadow-sm transition-all">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                            <h3 className="text-2xl font-black tracking-tight text-primary uppercase tracking-widest">Gastos por OS <span className="text-[10px] font-normal text-muted-foreground ml-2 tracking-normal lowercase italic">(Automático via Fluxo)</span></h3>
                            <p className="text-xs text-muted-foreground font-medium uppercase opacity-70">Consolidado de lançamentos vinculados a Ordens de Serviço</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card className="bg-emerald-500/5 border-emerald-500/20 border-l-4 border-l-emerald-500 p-6 flex flex-col gap-2 shadow-xl shadow-emerald-500/5 transition-all hover:translate-y-[-4px]">
                        <div className="flex justify-between items-start">
                            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600/70">Receita via OS</span>
                            <TrendingUp size={16} className="text-emerald-500" />
                        </div>
                        <span className="text-2xl font-black text-emerald-600">
                            {formatCurrency(totalAutoIncome)}
                        </span>
                        <p className="text-[9px] text-muted-foreground uppercase font-medium tracking-tight mt-1 truncate">Total de receitas vinculadas a OS no fluxo</p>
                    </Card>
                    ...
                    <Card className="bg-rose-500/5 border-rose-500/20 border-l-4 border-l-rose-500 p-6 flex flex-col gap-2 shadow-xl shadow-rose-500/5 transition-all hover:translate-y-[-4px]">
                        <div className="flex justify-between items-start">
                            <span className="text-[10px] font-black uppercase tracking-widest text-rose-600/70">Custos via OS</span>
                            <TrendingUp size={16} className="text-rose-500 rotate-180" />
                        </div>
                        <span className="text-2xl font-black text-rose-600">
                            {formatCurrency(totalAutoExpense)}
                        </span>
                        <p className="text-[9px] text-muted-foreground uppercase font-medium tracking-tight mt-1 truncate">Total de despesas vinculadas a OS no fluxo</p>
                    </Card>
                    ...
                    <Card className="bg-primary/5 border-primary/20 border-l-4 border-l-primary p-6 flex flex-col gap-2 shadow-xl shadow-primary/5 transition-all hover:translate-y-[-4px]">
                        <div className="flex justify-between items-start">
                            <span className="text-[10px] font-black uppercase tracking-widest text-primary/70">Resultado OS</span>
                            <DollarSign size={16} className="text-primary" />
                        </div>
                        <span className="text-2xl font-black text-primary">
                            {formatCurrency(totalAutoIncome - totalAutoExpense)}
                        </span>
                        <p className="text-[9px] text-muted-foreground uppercase font-medium tracking-tight mt-1 truncate">Lucratividade consolidada das OS</p>
                    </Card>
                </div>
                            ...
                            <table className="w-full text-xs border-collapse">
                                <thead>
                                    <tr className="bg-muted/50 text-muted-foreground border-b border-border/50 h-14">
                                        <th className="px-6 text-left font-black uppercase tracking-widest text-[10px]">Identificador OS</th>
                                        <th className="px-6 text-left font-black uppercase tracking-widest text-[10px]">Cliente</th>
                                        <th className="px-6 text-right font-black uppercase tracking-widest text-[10px]">Receita</th>
                                        <th className="px-6 text-right font-black uppercase tracking-widest text-[10px]">Despesa</th>
                                        <th className="px-6 text-right font-black uppercase tracking-widest text-[10px] bg-primary/5 text-primary">Saldo OS</th>
                                ...
                                <tbody>
                                    {automaticExpenses.length === 0 ? (
                                        <tr className="border-b border-border/30 h-32 text-center text-muted-foreground italic">
                                            <td colSpan={6}>Nenhuma transação do financeiro vinculada a Ordens de Serviço.</td>
                                        </tr>
                                    ) : (
                                        automaticExpenses.map((proj) => {
                                            const balance = proj.income - proj.expense;
                                            const margin = proj.income > 0 ? (balance / proj.income) * 100 : 0;
                                            return (
                                                <tr key={proj.id} className="border-b border-border/30 hover:bg-muted/30 transition-all group">
                                                    <td className="px-6 py-5 font-black text-sm uppercase text-foreground group-hover:text-primary transition-colors">{proj.id}</td>
                                                    <td className="px-6 py-5 font-bold text-xs uppercase text-muted-foreground">{proj.client}</td>
                                                    <td className="px-6 py-5 text-right font-semibold text-emerald-500">{formatCurrency(proj.income)}</td>
                                                    <td className="px-6 py-5 text-right font-semibold text-rose-500">{formatCurrency(proj.expense)}</td>
                                                    <td className="px-6 py-5 text-right font-black text-sm bg-primary/[0.03] group-hover:bg-primary/[0.08] transition-colors">{formatCurrency(balance)}</td>
            ...
            <div className="w-full h-px bg-border/20 shadow-sm" />
